...
//    }
    ...
    public JSONObject queryGatherPayment(Map<String, Object> map) throws Exception {
        ...
        if (payserviceBeans != null && payserviceBeans.size() > 0) {
            StringBuffer buffer = new StringBuffer();
//            buffer.append("SELECT * FROM ( ");
            buffer.append(" SELECT PM0.PAYMENT_ID,PM0.PATIENT_ID,PM0.PAYSERVICE_ID,PM0.RECEIVABLE,PM0.ACTUALPAYMENT,PM0.BEGTIME,PM0.ENDTIME,PM0.PAYMENTTIME ");
            for (int i=1;i<payserviceBeans.size();i++){
                String PM="PM"+String.valueOf(i);
            ...
            buffer.append(" FROM (SELECT * FROM PAYMENT WHERE PAYSERVICE_ID=1)PM0 ");
            for (int i=1;i<payserviceBeans.size();i++){
                String PM="PM"+String.valueOf(i);
                buffer.append(",(SELECT * FROM PAYMENT WHERE PAYSERVICE_ID="+(i+1)+")"+PM+" ");
            }
            buffer.append("WHERE PM0.PATIENT_ID=PM1.PATIENT_ID");
            for (int i=1;i<payserviceBeans.size()-1;i++){
                String PM="PM"+String.valueOf(i);
                String PM1="PM"+String.valueOf(i+1);
                buffer.append(" and "+PM+".PATIENT_ID="+PM1+".PATIENT_ID");
            }

            System.out.println("拼接SQL:    "+buffer.toString());
